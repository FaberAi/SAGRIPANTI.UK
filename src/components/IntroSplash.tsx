 ...
 * forma la scritta SAGRIPANTI in acciaio. Tutto su canvas, ~5,5s, poi
 * sfuma e rivela la landing. Si riproduce una volta per sessione.
 */
...
export default function IntroSplash() {
  ...
  useEffect(() => {
    ...
    const cx = W / 2;
    const cy = H / 2;
    let fs = Math.min(W * 0.13, 188);
    const fontOf = (s: number) => `900 ${s}px 'Arial Black','Helvetica Neue',Arial,sans-serif`;
    ...
    if (octx) {
      octx.font = fontOf(fs);
      while (octx.measureText(word).width > W * 0.9 && fs > 24) {
        fs -= 4;
        octx.font = fontOf(fs);
      }
      octx.fillStyle = "#fff";
    ...
    const frame = (now: number) => {
      ...
      if (t < steelStart) {
        ...
        // pioggia (sfuma durante la formazione)
